...
let PLATFORM = "douyin";
// 前端平台声明表。与后端 app/platforms/registry.py 对应（JS 读不到 Python，
// 加平台要两边都加一行；字段含义保持同名，方便对照）。
//   kind: video=视频/图集平台 | article=图文文章平台
//   monitor: 能否监控他人作品/评论（图文平台与视频号都不能）
const PF_SPECS = {
  douyin:    { label: "抖音",       kind: "video",   monitor: true,  publish: true },
  xhs:       { label: "小红书",     kind: "video",   monitor: true,  publish: true },
  kuaishou:  { label: "快手",       kind: "video",   monitor: true,  publish: true },
  shipinhao: { label: "视频号",     kind: "video",   monitor: false, publish: true },
...
function pfSpec(pf) { return PF_SPECS[pf] || PF_SPECS.douyin; }
// 图文平台：纯 HTTP 发布，只有「账号 + 发布」，没有监控/下载/弹幕那些
function pfIsArticle(pf) { return pfSpec(pf).kind === "article"; }
...
// 是否支持「发布」面板
function pfHasPublish(pf) { return !!pfSpec(pf).publish; }
// 只有「本账号」数据、不支持监控他人作品/评论的平台（视频号 + 三个图文平台）
function pfIsChannels(pf) { return !pfSpec(pf).monitor; }
function switchPlatform(pf) {
  if (!PF_KEYS.includes(pf)) pf = "douyin";
  PLATFORM = pf;
...
function applyPlatformUI() {
  PF_KEYS.forEach(k => document.body.classList.toggle("pf-" + k, PLATFORM === k));
  // 只有本账号数据的平台:隐藏「监控他人作品/评论」相关入口(.notsh-only)
  document.body.classList.toggle("pf-channels", pfIsChannels(PLATFORM));
  // 图文平台:没有媒体上传/画质/话题那一套,只有标题+正文+封面(.article-only / .video-only)
  document.body.classList.toggle("pf-article", pfIsArticle(PLATFORM));
  document.querySelectorAll(".article-only").forEach(e => e.classList.toggle("hidden", !pfIsArticle(PLATFORM)));
  document.querySelectorAll(".video-only").forEach(e => e.classList.toggle("hidden", pfIsArticle(PLATFORM)));
  ...
  const tkw = document.getElementById("ck-token-wrap");
  if (tkw) tkw.classList.toggle("hidden", PLATFORM !== "wechat_mp");   // token 输入只在公众号页显示
  ...
  const art = pfIsArticle(PLATFORM), pfLabel = PF_NAME[PLATFORM] || "当前平台";
  const pubSub = $("pub-head-sub");
  if (pubSub) pubSub.textContent = art ? `发布图文文章到${pfLabel}(纯协议,不开浏览器)`
    : dy ? "上传图集 / 视频到抖音创作平台(实验性)"
  ...
  const pubHintText =
    ...
    : PLATFORM === "baijiahao"
    ? "百家号走纯 HTTP 协议发布，不开浏览器。封面是平台必填项，只支持 1 张（单图版式）或 3 张（三图版式）；正文里的本地图会自动上传到百家号图床。AIGC 声明默认开启。"
    : PLATFORM === "toutiao"
    ? "今日头条走纯 HTTP 协议发布，不开浏览器。注意平台没有「新建草稿」这条路（save=0 新建会失败），所以这里提交即真发布。封面不给就是无封面文章——头条不会自动拿正文图当封面。"
    : dy
...
async function saveCookie() {
  ...
  const token = ($("ck-token") ? $("ck-token").value.trim() : "");
  if (PLATFORM === "wechat_mp" && !token) { toast("公众号还需要 token(后台 URL 的 token= 参数)", "err"); return; }
  try {
    const acc = await api("/api/login/cookie", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cookie, token, nickname: $("ck-nick").value.trim(), platform: PLATFORM }),
    });
    $("ck-val").value = ""; if ($("ck-token")) $("ck-token").value = "";
    $("cookiebox").style.display = "none";
    toast("Cookie 已保存", "ok"); refreshAccounts();
    // 图文平台顺手验一次登录态,贴错 cookie 当场就能发现
    if (pfIsArticle(PLATFORM) && acc && acc.account_id) {
      try {
        const chk = await api(`/api/accounts/${acc.account_id}/check-article-login`, { method: "POST" });
        const ok = chk.status === "active";
        toast(ok ? `登录验证通过:${chk.nickname || "已登录"}` : `登录验证失败:${chk.error || "cookie 可能不完整或已过期"}`, ok ? "ok" : "err");
        refreshAccounts();
      } catch (e) { toast("登录验证失败:" + e.message, "err"); }
    }
...
PLATFORM = (() => { try { const p = localStorage.getItem("dym-pf"); return PF_KEYS.includes(p) ? p : "douyin"; } catch (e) { return "douyin"; } })();
applyPlatformUI();
