...
用法:
    python -m application.channels.publish_cli <episode目录> [account_id] [--dry] [--no-original] [--no-ai]
...
from application.channels.api import (  # noqa: E402
    ChannelsAPI, build_topic_xml, cookies_from_profile, extract_topics,
    probe_video, resolve_finder_id)
from application.channels.publish import _clean_short_title  # noqa: E402
...
async def main():
    ...
    async with ChannelsAPI(cookie, finder_id, uin, UA, acc.proxy or None) as api:
        if dry:
            ...
            from application.channels.api import extract_cover
