...
from application.browser.account_hub import _douyin_cookie_str
from application.browser.douyin_im_ws import receive_messages
from moss.common.db import get_session
from moss.model import DmConversation, DmMessage, DouyinAccount
from contextlib import suppress
from moss.common.logging_setup import get_logger

log = get_logger("engine.im_receiver")
...
class ImReceiverManager:
    ...
    async def _run(self, account_id: int):
        ...
        for _ in range(15):
            with get_session() as s:
                ...
                if not acc or acc.platform != "douyin":
                    log.warning(f"[im-ws] _run account={account_id} 非抖音/不存在,跳过")
                    return
                uid = acc.uid
        ...
        log.debug(f"[im-ws] _run account={account_id} uid={uid!r}")
        if not uid:
            log.warning(f"[im-ws] account {account_id} 无 uid,点一次「同步」存下 uid 即自动接上")
            return
        try:
            cookie = await _douyin_cookie_str(self.browser, identity)
        except Exception as e:
            log.warning(f"[im-ws] account {account_id} 取 cookie 失败: {e!r}")
            return
