...
log = get_logger("browser.ks_fetcher")
...
async def fetch_ks_videos(mgr: BrowserManager, identity: Identity, user_id: str,
    ...
    if not collected:
        log.debug(f"[ks_videos] user_id={user_id!r} open_url={open_url!r} "
              f"final_url={final_url} api_seen({len(api_seen)})={api_seen[:40]}")
    # REST feed 命中但可能字段没对齐:打样本便于核对 _rest_item_to_feed
    if feed_sample:
        log.debug(f"[ks_videos] feed_sample={feed_sample[0]}")
...
async def fetch_ks_self_profile(mgr: BrowserManager, identity: Identity,
    ...
    if not result:
        ...
        log.warning(f"[ks_self_profile] 未拿到资料; err={error}; login_btn={has_login_btn}; uid_cookie={uid_cookie!r}; "
