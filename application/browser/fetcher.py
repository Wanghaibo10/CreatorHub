...
from application.douyin.extract import danmaku_key
from contextlib import suppress
from moss.common.logging_setup import get_logger

log = get_logger("browser.fetcher")
...
async def fetch_videos(mgr: BrowserManager, identity: Identity, sec_uid: str,
    ...
    if not collected:
        ...
        log.warning(f"[works] 未拿到作品; sec_uid={sec_uid[:24]}… final_url={final_url}; "
              f"post_hits({len(post_hits)})={post_hits[:5]}; dom={dom}")
        log.debug(f"[works] api_seen({len(api_seen)})={api_seen[:30]}")
    elif pagination_stalled:
        log.debug(f"[works] 主页分页未触发; sec_uid={sec_uid[:24]}… "
...
async def fetch_douyin_search(mgr: BrowserManager, identity: Identity, keyword: str,
    ...
    if not collected:
        log.debug(f"[dy_search] keyword={keyword!r} final_url={final_url!r} "
...
async def post_comment_browser(mgr: BrowserManager, identity: Identity, aweme_id: str,
    ...
    try:
        ...
        if editor is None:
            ...
            log.warning(f"[comment_post] 未找到输入框 aweme={aweme_id} diag={diag}")
...
async def _refetch_in_page(page, full_url: str) -> Optional[dict]:
    ...
    try:
        ...
        log.warning(f"[self_profile] refetch failed: {e!r}")
        return None
...
async def fetch_self_profile(mgr: BrowserManager, identity: Identity,
    ...
    if not result:
        ...
        log.warning(f"[self_profile] 未拿到资料; err={error}; final_url={final_url}; "
