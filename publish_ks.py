...
"""用纯接口发一条快手作品(零浏览器)。
    ...
    python publish_ks.py <视频> <文案> [--account 4] [--dry]
...
def _load_api():
    """按文件加载 api.py,不走 kuaishou 包(__init__ 会扯上 curl_cffi)。"""
    import importlib.util
    api_py = ROOT / "app" / "platforms" / "kuaishou" / "api.py"
    spec = importlib.util.spec_from_file_location("ks_protocol", api_py)
...
_ks = _load_api()
DEFAULT_UA = _ks.DEFAULT_UA
KuaishouAPI = _ks.KuaishouAPI
_ffprobe_duration_ms = _ks._ffprobe_duration_ms
cookies_from_account = _ks.cookies_from_account
sign_available = _ks.sign_available
