...
**添加账号**：顶部选平台 → 左侧「账号」→ 扫码 / 创作者登录 / Cookie 粘贴。图文平台点「打开登录页」会弹出平台官方后台，登录成功后自动抓取 Cookie 并验证（公众号的 `token` 也自动从 URL 提取）；也可以手动粘贴 Cookie。图文 Cookie 会**自动续期**：每次发文/体检顺带合并服务端新下发的 Cookie 回写；判活失败先带旧 Cookie 无头访问后台换新的一套（长效凭证活着就免重登）；真救不回来才标失效并推通知提醒重登。
